        ...
        try:
            if 'jpg' in file:
                ...
                if check_existing.exists():
                    ...
                    os.rename(
                        '/mnt/gam-dzis/{}'.format(file),
                        '/mnt/gam-dzis/{}'.format(front),
                    )

            else:
                new = file[:-3] + 'jpg.dzi'
                os.rename(
                    '/mnt/gam-dzis/{}'.format(file), '/mnt/gam-dzis/{}'.format(new)
                )
        except:
            print('exception')
        ...
        for file in os.listdir('/tmp/DIP/' + dip_name + '/objects/'):
            ...
            if file.split('.')[1] == 'jpg':
                ...
                if re.search('[a-zA-Z]', image):
                    ...
                    folder_images = Imagen.objects.filter(
                        archivo__id=archivo_id.pk,
                        colección__id=collection_id.pk,
                        caja=str(box),
                        legajo=str(bundle),
                        carpeta=str(folder),
                    ...
                    for image in folder_images:
                        image_letters = ''.join(
                            [i for i in image.número_de_imagen if not i.isdigit()]
                        )
                        image_numbers = ''.join(
                            [i for i in image.número_de_imagen if i.isdigit()]
                        )

                        image_id = Imagen.objects.get(
                            localizacion_fisica=physical_location
                        )
                        # print(image_id)
                        ...
                        if letters == image_letters:
                            ...
                            my_item = Item.objects.get(
                                nombre_del_item=archive
                                + '_'
                                + collection
                                + '_'
                                + box
                ...
                # Otherwise create a single-image item
                    ...
                    Item.objects.update_or_create(nombre_del_item=physical_location)
            ...
            if file.split('.')[1] == 'txt':
                ...
                for i in range(len(keys)):
                    try:
                        carpeta_descripcion[content[keys[i]]] = content[
                            (keys[i] + 1) : keys[i + 1]
                        ]
                    except IndexError:
                ...
                for carpeta in carpeta_descripcion:
                    ...
                    if archive == 'gam':
                        archivo_id = Archivo.objects.get(
                            nombre_del_archivo='Archivo del GAM'
                        )
                    else:
                        archivo_name = input(
                        ...
                        archivo_id = Archivo.objects.get(
                            nombre_del_archivo='%s' % archivo_name
                        )
                    ...
                    if collection == 'des':
                        ...
                        Colección.objects.get_or_create(
                            nombre_de_la_colección=collection
                        )
                        collection_id = Colección.objects.get(
