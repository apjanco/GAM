        ...
        try:
            if 'jpg' in file:
                ...
                if check_existing.exists():
                    ...
                    os.rename(
                        '/mnt/dzis1/{}'.format(file), '/mnt/dzis1/{}'.format(front)
                    )
...
def reformat_old_bag(dip_name):
    ...
    for file in os.listdir('/tmp/DIP/' + dip_name + '/objects/'):
        ...
        os.rename(
            '/tmp/DIP/{}/objects/{}'.format(dip_name, file),
            '/tmp/DIP/{}/objects/{}'.format(dip_name, rejoined),
        )
    ...
    def handle(self, *args, **options):
            ...
            if file.split('.')[1] == 'jpg':
                ...
                if collection == 'des':
                    ...
                    Colección.objects.get_or_create(
                        nombre_de_la_colección=collection_name
                    )
                    collection_id = Colección.objects.get(
        ...
        for index, image in enumerate(imagens_list):
            ...
            if re.search('[a-zA-Z]', número_de_imagen):
                ...
                if letters not in imagens_list[index - 1].split('.')[0].split('_')[-1]:
